...
- Manifest: `public/manifest.webmanifest` (id/scope/start_url set to `/`, maskable icons, shortcuts, portrait orientation)
- Service worker: generated by `vite-plugin-pwa` (Workbox) with precaching of hashed assets, a navigation fallback to `/index.html`, and runtime caching only for pages/images (API traffic is not cached).
- Icons: `public/icons/icon-192.png`, `public/icons/icon-512.png`

`index.html` links the manifest, adds theme color + Apple meta tags, and the service worker is registered through `ServiceWorkerManager` inside `src/App.tsx`. Users get a reload banner when a new version lands, and an "offline ready" toast when precaching is done.
