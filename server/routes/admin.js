...
const DEFAULT_PLATFORM_SETTINGS = {
  ...
  alerts: {
    low_balance_threshold_kes: 5000,
  },
};
...
function mergePlatformSettings(raw = {}) {
  ...
  return {
    fees: {
      platform_fee_pct: cleanNumber(raw?.fees?.platform_fee_pct ?? raw.platform_fee_pct, DEFAULT_PLATFORM_SETTINGS.fees.platform_fee_pct),
      sacco_fee_pct: cleanNumber(raw?.fees?.sacco_fee_pct ?? raw.sacco_fee_pct, DEFAULT_PLATFORM_SETTINGS.fees.sacco_fee_pct),
    },
    payout: {
      min_payout_kes: cleanNumber(raw?.payout?.min_payout_kes ?? raw.min_payout_kes, DEFAULT_PLATFORM_SETTINGS.payout.min_payout_kes),
      max_payout_kes: cleanNumber(raw?.payout?.max_payout_kes ?? raw.max_payout_kes, DEFAULT_PLATFORM_SETTINGS.payout.max_payout_kes),
    },
    alerts: {
      low_balance_threshold_kes: cleanNumber(
        raw?.alerts?.low_balance_threshold_kes ?? raw.low_balance_threshold_kes,
        DEFAULT_PLATFORM_SETTINGS.alerts.low_balance_threshold_kes
      ),
    },
  };
}
...
async function getPlatformSettings() {
  ...
  return {
    settings: mergePlatformSettings(raw),
    updated_at: rows[0]?.updated_at || null,
    updated_by: rows[0]?.updated_by || null,
  };
}

async function savePlatformSettings(payload, userId) {
  const settings = mergePlatformSettings(payload || {});
  await pool.query(
    `
...
async function logAudit(req, action, meta = {}, entityType = null, entityId = null) {
  try {
    await pool.query(
      `
        insert into admin_audit_logs
          (user_id, action, entity_type, entity_id, path, meta, ip_address)
        values
          ($1, $2, $3, $4, $5, $6, $7)
      `,
      [
...
async function upsertAlert({ dedupeKey, alertType, severity = 'INFO', message, meta = {} }) {
  const { rows } = await pool.query(
    `
      insert into system_alerts (dedupe_key, alert_type, severity, message, meta, status)
      ...
      returning *
    `,
    [dedupeKey || null, alertType, severity, message, meta]
  );
  return rows[0];
}

async function resolveAlertByDedupe(dedupeKey) {
...
async function refreshSystemAlerts(settings) {
  const alerts = [];

  // Low balance on system wallet
  const lowBalanceThreshold = Number(settings?.alerts?.low_balance_threshold_kes || 0);
  if (lowBalanceThreshold > 0) {
    const { rows } = await pool.query(
      `select id, balance, virtual_account_code from wallets where entity_type = 'SYSTEM' order by created_at asc limit 1`
    );
    if (rows.length) {
      const sys = rows[0];
      const bal = Number(sys.balance || 0);
      const key = `low_balance:${sys.id}`;
      if (bal < lowBalanceThreshold) {
        alerts.push(
          await upsertAlert({
            ...
            severity: 'WARN',
            message: `System wallet ${sys.virtual_account_code || sys.id} is low: KES ${bal.toFixed(
              2
            )} < threshold KES ${lowBalanceThreshold}`,
            meta: { wallet_id: sys.id, balance: bal, threshold: lowBalanceThreshold },
          })
        );
      } else {
        await resolveAlertByDedupe(key);
      }
    }
  }
  ...
  for (const row of failedWithdrawals) {
    alerts.push(
      await upsertAlert({
        dedupeKey: `failed_withdrawal:${row.id}`,
        alertType: 'FAILED_WITHDRAWAL',
        severity: 'CRITICAL',
        message: `Withdrawal ${row.id} failed for KES ${Number(row.amount || 0).toFixed(2)} to ${
          row.phone_number || 'unknown'
        }`,
        meta: { withdrawal_id: row.id, amount: Number(row.amount || 0), phone: row.phone_number, status: row.status },
      })
    );
  }
  ...
  res.on('finish', async () => {
    try {
      if (['POST', 'PUT', 'PATCH', 'DELETE'].includes(req.method)) {
        await logAudit(req, `${req.method} ${req.path}`, {
      ...
      console.error('audit log error (finish):', err.message);
    }
  });
  next();
});
...
router.post('/platform-settings', async (req, res) => {
  try {
    const settings = await savePlatformSettings(req.body || {}, req.user?.id || null);
    await logAudit(req, 'platform_settings.update', { settings });
...
router.get('/system-alerts', async (req, res) => {
  try {
    ...
    await refreshSystemAlerts(settings);
    ...
    const { rows } = await pool.query(
      `
        select a.*, sp.name as resolved_by_name
        from system_alerts a
        left join staff_profiles sp on sp.user_id = a.resolved_by
        ${where}
...
router.post('/system-alerts/:id/resolve', async (req, res) => {
  const { id } = req.params || {};
  try {
    const { rows } = await pool.query(
      `
    ...
    await logAudit(req, 'system_alert.resolve', { alert_id: id });
